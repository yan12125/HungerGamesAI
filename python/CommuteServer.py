...
class DODOWebSocket(WebSocket):
  def received_message(self, message):
      global Message
      myID=eval(message.data)['myPlayerid']
      print myID
      if Message:
        for item in Message:
          if eval(item.data)['friendPlayerid']==myID:
            self.send(item.data, item.is_binary)
            Message.remove(item)
      if message.data != None and eval(message.data)['friendPlayerid']!='None':
        m=deepcopy(message)
        Message.append(m)
...
if __name__ == '__main__':
    ...
    server = WSGIServer(('127.0.0.1', 6000),
                        WebSocketWSGIApplication(handler_cls=DODOWebSocket))
    server.serve_forever()
